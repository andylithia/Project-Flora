...
uint16_t EE_Init(void) {
    ...
    /* --------------------------- PAGE_0 erased ---------------------------- */
	case ERASED:
		if(ps1 == VALID_PAGE) return EE_CheckData();	// PAGE_1 valid
		else if(ps1 == RECEIVE_DATA) {					// PAGE_1 receive
			/* erase PAGE_0 */
			uint32_t pageError;
			FLASH_EraseInitTypeDef EraseInitInst;
			EraseInitInst.PageAddress = PAGE_0_BASE_ADDRESS;
			EraseInitInst.TypeErase = FLASH_TYPEERASE_PAGES;
			EraseInitInst.NbPages = 1;
			status = HAL_FLASHEx_Erase(&EraseInitInst, &pageError);
			if(status != HAL_OK) return status;

			/* mark PAGE_1 as valid */
			status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, PAGE_1_BASE_ADDRESS, VALID_PAGE);
			if(status != HAL_OK) return status;
		} else										// invalid state
	...
	case RECEIVE_DATA:
      ...
      // TODO: Modify the following part to fit in HAL
    /* --------------------------- PAGE_0 valid ----------------------------- */
    case VALID_PAGE:
